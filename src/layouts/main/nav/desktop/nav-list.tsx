...
export default function NavList({ item }: { item: NavItemBaseProps }) {
  ...
  const mainList = item.children
    ? item.children.filter((list) => list && list.subheader && list.subheader !== 'Common')
    : [];

  const commonList = item.children
    ? item.children.find((list) => list && list.subheader === 'Common')
    : null;
  ...
  return (
    <Stack spacing={2}>
      ...
      <Stack spacing={1.5} alignItems="flex-start">
        {items.map((item) => {
          const active = item.path ? pathname === item.path || pathname === `${item.path}/` : false;
